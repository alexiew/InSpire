...
export function createSubscription(sourceType: string, sourceIdentifier: string, name: string, extractionHints?: string, excludeTerms?: string): Subscription {
  const db = getDb();
  const now = new Date().toISOString();

  const result = db
    .prepare(
      `INSERT INTO subscriptions (source_type, source_identifier, name, extraction_hints, exclude_terms, subscribed_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    )
    .run(sourceType, sourceIdentifier, name, extractionHints || "", excludeTerms || "", now);
